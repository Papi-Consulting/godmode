...
| Persistence | SQLite via better-sqlite3 (JSON-file fallback) | Per-operated-project run store (`.godmode/godmode.db`); resume after restart (issue #40) |
...
The live PTY sessions exist only in main's process memory, so the main process is the single source of truth for each pane's **session lifecycle** (issue #63), not the renderer's optimistic button clicks. A `PaneSessionState` per role pane distinguishes `never_started` / `running` / `exited` (with the real exit code) / `stopped` (operator- or project-switch-ended) / `failed` (launch error, with a visible reason), plus pid, launch cwd, and a conservative `awaitingInput` attention flag. Every launch path (renderer Start, builder recovery relaunch, reviewer launch) funnels through the same `pty.ts` registry, which pushes a fresh snapshot to the renderer (`godmode:pty:state`, fetched on mount via `godmode:pty:state:get`). Pane headers, Start/Restart/Stop controls, and the message input all read this truth: a one-shot reviewer that exits shows `exited (code)` rather than a live watcher, and the message Send control is disabled when no live PTY exists. The `awaitingInput` flag is a deliberately heuristic, vendor-neutral prompt detector (`detectPromptAttention`) — it only surfaces a "needs operator" hint, never gates delivery, and clears on the next output or input. See `docs/architecture/pane-session-state.md`.

Each pane also has an operator **`Message <role>`** control that delivers the exact text plus one submit character (`\r`) to that role's live PTY via a typed `sendPty` IPC (`godmode:pty:send` → `writeToPtySessionResult`). The field clears **only** on a confirmed write; a `no_live_session`/`unknown_pane`/`write_failed` result is shown inline and the text is preserved to retry (issue #57) — never a silent no-op. Raw xterm typing keeps the fire-and-forget `writePty` path. The global command bar (spec §6.5) and harness team-chat input have no designed routing yet, so they are rendered visibly disabled rather than accepting text they cannot dispatch. See `docs/architecture/operator-command-dispatch.md`.
...
The issue-to-PR workflow is governed by a single deterministic run state machine in the main process, not by agent self-report or by transition rules scattered across the UI. A `RunSnapshot` carries the current status, selected issue, branch/PR, cycle/`maxCycles`, any blocker/reason, the available operator actions, and an append-only transition log. Every phase change goes through one guard (`applyAction`): it consults a central transition table, rejects illegal transitions with a typed error and **no** state mutation, and on success logs the transition (`from`/`to`/`action`/`reason`) and recomputes `availableActions`. The dashboard renders operator controls from `availableActions` and selects an issue from the GitHub pane via `godmode:run:*` IPC (Zod-validated). Spec states beyond the `RunStatus` union are reconciled explicitly: `karan_merged` and `closed` are added as terminal statuses, while `pr_conflicted`, `tests_failed`, `checks_unstable`, `harness_missing`, and `repo_dirty` map onto `needs_human` with a recorded `blocker`. The snapshot is **persisted write-through** to a per-operated-project run store (`.godmode/godmode.db`, SQLite via better-sqlite3 with a JSON-file fallback) on every accepted mutation, so quitting GodMode mid-run never loses it: on relaunch, reselecting the project offers an explicit **Resume/Discard** choice (GodMode never auto-resumes). A resumed run is restored through the state machine — live PTY/reviewer sessions are marked dead, `availableActions` is recomputed, and a recorded PR that no longer matches GitHub routes the run to `needs_human`. A persistence failure (e.g. a read-only project) degrades to in-memory with a visible warning rather than crashing. Because live PTY sessions live only in main-process memory, a reset or restart can leave a run persisted as `builder_running` while the builder PTY is gone; GodMode detects that stale state, surfaces it distinctly (never a bare `blocked`), and offers explicit recovery — relaunch the builder and re-deliver the existing handoff (under the same worktree cwd safety gate, recorded for audit, refused while a builder is still live), or mark the agent failed — without ever auto-relaunching. See `docs/architecture/run-state-machine.md`, `docs/architecture/run-persistence.md`, and `docs/architecture/builder-recovery.md`.
